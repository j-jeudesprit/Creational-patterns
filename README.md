...
Иногда, вместо того что бы инстанцировать новый объект и долго его настраивать, лучшем вариантом будет просто получить его копию. Такую необходимость и решает паттерн **Прототип**.   
...
Несмотря на то, что паттерн больше является анти-паттерном, т.к. противоречит принципам SOLID и является больше STUPID, всё же представление о нём иметь надо.   
  
Для его реализации, мы сделаем свойство, которое будет содержать экземпляр - константным, что не позволит перезаписать его, а инициализатор класса - приватным, что бы никто не смог создать ещё один экземпляр. Посмотрим:  
