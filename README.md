...
Теперь мы везде пользуемся лишь интерфейсами, что не ограничивает нас в выборе того или иного транспорта.  Добавление же нового транспорта не приводит к изменения кода, который использует эти их. Но возникаем небольшая проблема, как интерфейс работающий с любым транспортом может знать, какой именно ему нужно создавать транспорт. Управлять он ими может, а вот когда и какой ему нужно будет создать он не знает. Решить эту проблему помогает **Фабричный метод**.  
...
Теперь для того что бы создать нужный нам транспорт, достаточно вызвать метод *createTransport()*, которые всегда вернет нужный нам транспорт. Таких фабричных методов в вашем классе может быть несколько. Теперь создание и управление нашим транспортом выглядит совершенно одинаково в ваше коде для любого его вида:  
