...
// Можем проверять какой-то параметр, который   
можно передать Директору.
// В целом Директор должен уметь строить абсолютно  
любой дом, который мы у него попросим.
// А как он это будет делать, волновать нас не   
должно.  
